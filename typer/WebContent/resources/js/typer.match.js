(function($) {
	...
	var betPastTemplate = '<div class="bet">BET_PLACEHOLDER</div>';
	...
	var winnerBetPastTemplate = '<div class="row"><div class="col-md-1"><span>Typ na mistrza:</span></div><div class="col-md-2">WINNER_PLACEHOLDER</div></div>';
	var flagMap = {
		'Brazylia' : 'bra.png',
		'Chorwacja' : 'cro.png',
		'Meksyk' : 'mex.png',
	...
	$.fn.addWinnerBetDropdown = function(options) {
		...
		var getBetHtml = function(data) {
			...
			if (data.winner) {
				...
				var pointsTemplate = data.winner.name == data.bet.name ? pointsTemplateMap['3'] : pointsTemplateMap['0'];
				html = winnerBetPastTemplate.replace('WINNER_PLACEHOLDER', pointsTemplate.replace('CONTENT_PLACEHOLDER', data.bet.name));
				
			} else if (currentDate > deadlineDate) {
				
				html = winnerBetPastTemplate.replace('WINNER_PLACEHOLDER', pointsTemplateMap['undefined'].replace('CONTENT_PLACEHOLDER', data.bet.name));
				...
				$.each(data.teams, function(key, value) {
					teamListHtml += '<li class="team"><a href="#">' + value.name
							+ '</a></li>';
				});
				
				html = html.replace('WINNER_LIST_PLACEHOLDER', teamListHtml)
					.replace('ORIGINAL_BET_PLACEHOLDER', data.bet ? data.bet.name : '')
					.replace('ORIGINAL_DEADLINE_PLACEHOLDER', new Date(data['deadline']))
					.replace('DEADLINE_PLACEHOLDER', $.formatDateTime('yy-mm-dd hh:ii', new Date(data.deadline)));
		...
		$.fn.setBetTimeout = function() {
			...
			var deadline = new Date($('#winner-bet-deadline').attr('data-datetime'));
